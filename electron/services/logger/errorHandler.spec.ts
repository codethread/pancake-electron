...
describe('_errorHandler', () => {
  ...
  let versions: undefined | typeof defaultVersions;
  const submitIssue = jest.fn();

  beforeEach(() => {
    const appliedErrorHandler = errorHandler(logger);
    appliedErrorHandler && appliedErrorHandler(error, versions, submitIssue);
  });
  ...
  describe('when the user choses to ignore the error', () => {
    beforeAll(() => {
